    ...
    public class ToggleGroupController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            for (int i = 0; i < _togglePairs.Count; i++)
            {
                ...
                if(_togglePairs[i].toggle.TryGetComponent<ButtonAnimator>(out var animator))
                {
                    _togglePairs[i].animator = animator;
                }
            }
            if (_selectOnEnable)
            {
        ...
        private void SelectElement(int index)
        {
            for (int i = 0; i < _togglePairs.Count; i++)
            {
                ...
                if (_togglePairs[i].animator != null)
                {
                    _togglePairs[i].animator.ForcePressed(false);
                }
            ...
            if (_togglePairs[index].animator != null)
            {
                _togglePairs[index].animator.ForcePressed(true);
            }
